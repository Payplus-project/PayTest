#!/bin/sh
#Version 0.0.2
#Info: Build paytest wallet for wins
#PayTestcoin Version 4.0.0 or above
#Tested OS: Ubuntu 17.04, 16.04, 14.04 & Linux-Mint-18
#TODO: remove dependency on sudo user account to run script (i.e. run as root and specifiy PAYTEST user so PAYTEST user does not require sudo privileges)
#TODO: add specific dependencies depending on build option (i.e. gui requires QT5)

noflags() {
	echo "┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄"
    echo "Usage: Auto configure and build PAYTEST wallet for Windows"
...
prepdependencies() { #TODO: add error detection
	message "Installing PAYTEST dependencies..."
	sudo apt-get update
...
clonerepo() { #TODO: add error detection
	message "Cloning PAYTEST source from master repository..."
  	cd ~/
...
compile() {
	cd paytest #TODO: squash relative path
	   message "Preparing HOST mingw32 to build ...this may take a few minutes..."
        ...
        cd .. && CONFIG_SITE=$PWD/depends/x86_64-w64-mingw32/share/config.site
           if [ $? -ne 0 ]; then error; fi
    	   message "Build PAYTEST...this may take a few minutes..."
        ./autogen.sh
           if [ $? -ne 0 ]; then error; fi
...
createconf() {
	...
	#TODO: Random generate PAYTEST user/password

	   message "Creating paytest.conf..."
	CONFDIR=~/.paytest
	CONFILE=$CONFDIR/paytest.conf
	   if [ ! -d "$CONFDIR" ]; then mkdir $CONFDIR; fi
	   if [ $? -ne 0 ]; then error; fi	
	mnip=$(curl -s https://api.ipify.org)
	rpcuser=$(date +%s | sha256sum | base64 | head -c 10 ; echo)
	rpcpass=$(openssl rand -base64 32)
	printf "%s\n" "rpcuser=$rpcuser" "rpcpassword=$rpcpass" "rpcallowip=127.0.0.1" "port=37373" "listen=1" "server=1" "daemon=1" "maxconnections=256" "rpcport=37372" > $CONFILE
}
