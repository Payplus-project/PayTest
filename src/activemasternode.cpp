...
//
// Bootup the masternode, look for a 500 PayTest input and register on the network
//
...
bool CActiveMasternode::Dseep(CTxIn vin, CService service, CKey keyMasternode, CPubKey pubKeyMasternode, std::string &retErrorMessage, bool stop) {
    ...
    if(!found){
    	...
    	retErrorMessage = "PayTestsend Masternode List doesn't include our masternode, Shutting down masternode pinging service! " + vin.ToString();
    	LogPrintf("CActiveMasternode::Dseep() - Error: %s\n", retErrorMessage.c_str());
        status = MASTERNODE_NOT_CAPABLE;
...
vector<COutput> CActiveMasternode::SelectCoinsMasternode() {
    ...
    BOOST_FOREACH(const COutput& out, vCoins) {
        if(out.tx->vout[out.i].nValue == GetMNCollateral(chainActive.Tip()->nHeight)*COIN) {  //exactly 15000 PAYTEST
...
//    scriptPubKey.SetDestination(address.Get());
    ...
    // Filter
    BOOST_FOREACH(const COutput& out, vCoins) {
        if(out.tx->vout[out.i].scriptPubKey == scriptPubKey && out.tx->vout[out.i].nValue == 15000*COIN) { //exactly 150.000 PAYTEST
        	filteredCoins.push_back(out);
        }
    }
    return filteredCoins;
