...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::PAYTEST)
    {
    }
    ...
    if (fLiteMode) {
        ...
        if (fMasterNode) {
            ...
            if (!fEnablePayTestsend) {
                ui->toggleObfuscation->setText(tr("Start PayTestsend"));
            } else {
                ui->toggleObfuscation->setText(tr("Stop PayTestsend"));
            }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("PAYTEST")
    updateDisplayUnit();
}
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizePayTestAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizePayTestAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizePayTestAmount = strAnonymizePayTestAmount.remove(strAnonymizePayTestAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizePayTestAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizePayTestAmount * COIN) nMaxToAnonymize = nAnonymizePayTestAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizePayTestAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizePayTestAmount));
        strAnonymizePayTestAmount = strAnonymizePayTestAmount.remove(strAnonymizePayTestAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizePayTestAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleObfuscation()
{
    ...
    fEnablePayTestsend = !fEnablePayTestsend;
    obfuscationPool.cachedNumBlocks = std::numeric_limits<int>::max();
