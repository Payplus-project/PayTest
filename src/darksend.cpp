...
std::vector<CPayTestsendQueue> vecPayTestsendQueue;
/** Keep track of the used masternodes */
std::vector<CTxIn> vecMasternodesUsed;
// keep track of the scanning errors I've seen
map<uint256, CPayTestsendBroadcastTx> mapPayTestsendBroadcastTxes;
//
...
void ProcessPayTestsend(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, bool &isPayTestsend)
{
    ...
    if (strCommand == "dsf") { //DarkSend Final tx
        ...
        if(darkSendPool.sessionUsers == 0) {
            if(vecMasternodes[mn].nLastDsq != 0 &&
                ...
                std::string strError = _("Last PayTestsend was too recent.");
                pfrom->PushMessage("dssu", darkSendPool.sessionID, darkSendPool.GetState(), darkSendPool.GetEntriesCount(), MASTERNODE_REJECTED, strError);
                return;
            }
        }
        ...
        if(dsq.ready) {
            ...
            darkSendPool.PreparePayTestsendDenominate();
        } else {
            BOOST_FOREACH(CPayTestsendQueue q, vecPayTestsendQueue){
            ...
            vecPayTestsendQueue.push_back(dsq);
            dsq.Relay();
            dsq.time = GetTime();
        }
		...
		if(GetTransaction(i.prevout.hash, tx2, hash)){
            ...
            if (nValueIn > DARKSEND_POOL_MAX) {
                ...
                error = _("Value more than PayTestsend pool maximum allows.");
                pfrom->PushMessage("dssu", darkSendPool.sessionID, darkSendPool.GetState(), darkSendPool.GetEntriesCount(), MASTERNODE_REJECTED, error);
                return;
            }
...
int GetInputPayTestsendRounds(CTxIn in, int rounds)
{
    ...
    if (wtx != NULL) {
        ...
        BOOST_FOREACH(CTxIn in2, wtx->vin)
        {
            if(pwalletMain->IsMine(in2))
            {
                ...
                int n = GetInputPayTestsendRounds(in2, rounds+1);
                if(n != -3) return n;
            }
        }
    }
...
//
// Unlock coins after PayTestsend fails or succeeds
//
...
//
// Check the PayTestsend progress and send client updates if a masternode
//
    ...
    if(state == POOL_STATUS_SIGNING && SignaturesComplete()) {
        ...
        LOCK2(cs_main, pwalletMain->cs_wallet);
        {
            if (fMasterNode) { //only the main node is master atm
                ...
                if(!mapPayTestsendBroadcastTxes.count(txNew.GetHash())){
                    CPayTestsendBroadcastTx dstx;
                    dstx.tx = txNew;
                    dstx.vin = activeMasternode.vin;
                    dstx.vchSig = vchSig;
                    dstx.sigTime = sigTime;

                    mapPayTestsendBroadcastTxes.insert(make_pair(txNew.GetHash(), dstx));
                }
...
//
// Why bother? PayTestsend uses collateral to ensure abuse to the process is kept to a minimum.
...
//  - PayTestsend is completely free, to pay miners we randomly pay the collateral of users.
    ...
    if(fMasterNode) {
        ...
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat PayTest and make it unusable. To
                stop these kinds of attacks 1 in 50 successful transactions are charged. This
                adds up to a cost of 0.002PayTest per transaction on average.
            */
            if(r <= 20)
            {
...
void CDarkSendPool::CheckTimeout(){
    if(!fEnablePayTestsend && !fMasterNode) return;
    ...
    int c = 0;
    vector<CPayTestsendQueue>::iterator it;
    for(it=vecPayTestsendQueue.begin();it<vecPayTestsendQueue.end();it++){
        if((*it).IsExpired()){
            if(fDebug) LogPrintf("CDarkSendPool::CheckTimeout() : Removing expired queue entry - %d\n", c);
            vecPayTestsendQueue.erase(it);
            break;
        }
        c++;
    }
...
//
void CDarkSendPool::SendPayTestsendDenominate(std::vector<CTxIn>& vin, std::vector<CTxOut>& vout, int64_t amount){
    if(darkSendPool.txCollateral == CMutableTransaction()){
        LogPrintf ("CPayTestsendPool:SendPayTestsendDenominate() - PayTestsend collateral not set");
        return;
    }
    ...
    if(!sessionFoundMasternode){
        LogPrintf("CDarkSendPool::SendPayTestsendDenominate() - No masternode has been selected yet.\n");
        UnlockCoins();
        SetNull(true);
        return;
    }
    ...
    if(fMasterNode) {
        LogPrintf("CDarkSendPool::SendPayTestsendDenominate() - DarkSend from a masternode is not supported currently.\n");
        return;
    }

    UpdateState(POOL_STATUS_ACCEPTING_ENTRIES);

    LogPrintf("CDarkSendPool::SendPayTestsendDenominate() - Added transaction to pool.\n");
...
// PayTestsend transaction was completed (failed or successed)
...
//
// Passively run PayTestsend in the background to anonymize funds based on the given configuration.
//
...
bool CDarkSendPool::DoAutomaticDenominating(bool fDryRun, bool ready)
{
    ...
    if(!fEnablePayTestsend) {
        if(fDebug) LogPrintf("CDarkSendPool::DoAutomaticDenominating - PayTestsend is disabled\n");
        strAutoDenomResult = _("PayTestsend is disabled.");
        return false;
    }
    ...
    int64_t nBalanceNeedsAnonymized = nAnonymizePayTestAmount*COIN - pwalletMain->GetAnonymizedBalance();
...
//        }
    ...
    if(!sessionFoundMasternode){
        ...
        double fPayTestSubmitted = (sessionTotalValue / CENT);
        LogPrintf("Submitting PayTestsend for %f PayTest CENT - sessionTotalValue %d\n", fPayTestSubmitted, sessionTotalValue);
        ...
        if(nUseQueue > 33){
            ...
            BOOST_FOREACH(CPayTestsendQueue& dsq, vecPayTestsendQueue){
                CService addr;
...
bool CDarkSendPool::PreparePayTestsendDenominate()
{
    // Submit transaction to the pool if we get here, use sessionDenom so we use the same amount of money
    std::string strError = pwalletMain->PreparePayTestsendDenominate(0, nDarksendRounds);
...
void CDarkSendPool::GetDenominationsToString(int nDenom, std::string& strDenom){
    ...
    //
    // bit 0 - 100PayTest+1 ( bit on if present )
    // bit 1 - 10PayTest+1
    // bit 2 - 1PayTest+1
    // bit 3 - .1PayTest+1
    // bit 3 - non-denom
...
bool CPayTestsendQueue::Sign()
{
    ...
    {
        LogPrintf("CPayTestsendQueue():Relay - ERROR: Invalid masternodeprivkey: '%s'\n", errorMessage.c_str());
        return false;
    }

    if(!darkSendSigner.SignMessage(strMessage, errorMessage, vchSig, key2)) {
        LogPrintf("CPayTestsendQueue():Relay - Sign message failed");
        return false;
    }

    if(!darkSendSigner.VerifyMessage(pubkey2, vchSig, strMessage, errorMessage)) {
        LogPrintf("CPayTestsendQueue():Relay - Verify message failed");
        return false;
    }
...
bool CPayTestsendQueue::CheckSignature()
{
    BOOST_FOREACH(CMasterNode& mn, vecMasternodes) {
        ...
        if(mn.vin == vin) {
            ...
            if(!darkSendSigner.VerifyMessage(mn.pubkey2, vchSig, strMessage, errorMessage)){
                return error("CPayTestsendQueue::CheckSignature() - Got bad masternode address signature %s \n", vin.ToString().c_str());
            }
        ...
        if(c % 60*(nLiquidityProvider+1) == 0){
            if(nLiquidityProvider!=0){
                ...
                if(nRand == 50+nLiquidityProvider && pwalletMain->GetAverageAnonymizedRounds() > 8){
                    ...
                    nAnonymizePayTestAmount = (rand() % nLeftToAnon)+3;
                } else {
