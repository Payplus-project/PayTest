...
class CPayTestsendQueue;
class CPayTestsendBroadcastTx;
...
extern std::vector<CPayTestsendQueue> vecPayTestsendQueue;
extern std::string strMasterNodePrivKey;
extern map<uint256, CPayTestsendBroadcastTx> mapPayTestsendBroadcastTxes;
extern CActiveMasternode activeMasternode;

//specific messages for the PayTestsend protocol
...
int GetInputPayTestsendRounds(CTxIn in, int rounds=0);
...
public:
    ...
    // Passively run PayTestsend in the background according to the configuration in settings (only for QT)
    bool DoAutomaticDenominating(bool fDryRun=false, bool ready=false);
    bool PreparePayTestsendDenominate();


    // check for process in PayTestsend
    void Check();
    ...
    void SendPayTestsendDenominate(std::vector<CTxIn>& vin, std::vector<CTxOut>& vout, int64_t amount);
    ...
    // get the denominations for a specific amount of PayTest.
