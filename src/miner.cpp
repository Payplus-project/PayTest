...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("PAYTESTMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("PAYTESTMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("PAYTESTMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("paytest-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running PAYTESTMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
