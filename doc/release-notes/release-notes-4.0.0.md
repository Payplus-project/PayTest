PAYTEST Core version 4.0.0 is now available from:
...
PAYTEST Core v4.0.0 is a recommended, semi-mandatory update for all users. This release contains transaction creation bug fixes for zPAYTEST spends, automint calculation adjustments, and other various updates/fixes.

zPAYTEST spending requires this update.
...
Auto Wallet Backup
---------------------
In addition to the automatic wallet backup that is done at each start of the client, a new automatic backup function has been added that will, by default, create a backup of the wallet file during each zPAYTEST mint operation (zPAYTEST spends which re-mint their change are also included in this). This functionality is controlled by the `-backupzpaytest` command-line option, which defaults to `1` (enabled, auto-backup).

Users that wish to prevent this behavior (not recommended) can pass `-backupzpaytest=0` at the command-line when starting the client, or add `backupzpaytest=0` to their `paytest.conf` file.

zPAYTEST Automint Calculations
---------------------
A bug in the automint calculations was made apparent on mainnet when block times exceeded expectations, resulting in zPAYTEST mint transactions that were in an unconfirmed state to still be treated as if they had never been minted. This caused automint to effectively mint more than what was intended.

zPAYTEST Spending Fix
---------------------
The size of zPAYTEST spend transactions is knowingly larger than normal transactions, and while this was expected, a much stricter check against the scriptsig size is used for mainnet, causing the transactions to be rejected by the mempool, and thus not being packaged into any blocks.

zPAYTEST Transaction Recovery
---------------------
Due to the aforementioned issue with zPAYTEST spending, users may find that their attempted spends are now conflicted and zPAYTEST balances are not represented as expected. "Recovery" of these transactions can be done using the following methods:
...
RPC Changes
---------------------
The `bip38decrypt` command has had it's parameter order changed to be more consistent with it's counterpart. The command now expects the PAYTEST address as it's first parameter and the passphrase as it's second parameter.
...
- #292 `feadab4` Additional checks for double spending of zPayTest serials. (presstab)

### Wallet
- #271 `5e9a086` [Wallet] Remove unused member wallet in UnlockContext inner class (Jon Spock)
- #279 `e734010` Add -backupzpaytest startup flag. (presstab)
- #280 `fdc182d` [Wallet] Fix zPayTest spending errors. (presstab)
- #282 `4.0.016` [Wallet] Count pending zPayTest balance for automint. (presstab)
...
- #291 `cb314e6` [Qt] zPayTest control quantity/amount fixes (rejectedpromise)
